...
int main() {
    ...
    cout << "Enter the Major Radius (the distance from "\
    "the center of the tube to the center of the torus) ";
    cin >> majRadius;
    ...
    if (majRadius > minRadius) {
        ...
        volume = (2 / 3) * M_PI * (2 * pow(minRadius, 2) \
        + pow(majRadius, 2)) * sqrt(pow(minRadius, 2) - \
        pow(majRadius, 2)) + M_PI * pow(minRadius, 2) * \
            majRadius * (M_PI + 2 * atan(majRadius / \
            sqrt(pow(minRadius, 2) - pow(majRadius, 2))));
        cout << "The volume is "<< volume \
        <<" and the surface area is "<< surfaceArea<< "\n";
    }
    string tryAgain;
